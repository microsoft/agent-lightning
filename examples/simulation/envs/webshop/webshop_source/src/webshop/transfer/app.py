...
def bert_predict(obs, info, softmax=True):
    ...
    action_encodings = bert_tokenizer(
        list(map(process_str, valid_acts)), max_length=512, truncation=True, padding="max_length"
    )
    batch = {
...
def get_return_value(env, asin, options, search_terms, page_num, product):
    ...
    if env == "webshop":
        ...
        asin_url = f"{WEBSHOP_URL}/item_page/{WEBSHOP_SESSION}/" f"{asin}/{query_str}/{page_num}/{options_str}"
    else:
...
gr.Interface(
    ...
    outputs=[gr.outputs.JSON(label="Selected Product"), gr.outputs.JSON(label="Selected Options"), gr.outputs.HTML()],
    examples=[
        [
