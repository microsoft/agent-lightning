...
class SimServer:
    ...
    def search_results(self, session_id, **kwargs):
        ...
        url = f"{self.base_url}/search_results/{session_id}/" f"{keywords_url_string}/{page}"
    ...
    def done(self, session_id, **kwargs):
        ...
        url = f"{self.base_url}/done/{session_id}/" f'{session["asin"]}/{session["options"]}'
        html = map_action_to_html(
            f"click[{END_BUTTON}]",
            session_id=session_id,
            reward=reward,
            ...
            if not kwargs:
                ...
                if clickable_name == END_BUTTON.lower():
                    ...
                    html, url = self.search_results(
                        session_id, keywords=session["keywords"], page=session["page"], **kwargs
                    )
