...
from agentlightning.tracer.phoenix import PhoenixTracer

from ..common.tracer import clear_agentops_init, clear_tracer_provider


def create_span(
    name: str, sampled: bool = True, with_context: bool = True
) -> MagicMock:
...
def test_concurrent_access():
    ...
    with processor.with_context(
        store=store, rollout_id="concurrent", attempt_id="test"
    ):
        threads = [
            threading.Thread(target=process_spans, args=(i,))
...
async def _otel_reward_subprocess_async(
    mode: str, conn: Connection[tuple[str, Any]]
) -> None:
    ...
    try:
            ...
            async with tracer.trace_context(name="reward-decorator"):
                ...
                if returned != expected_reward:
                    raise AssertionError(
                        f"Expected reward {expected_reward}, got {returned}"
                    )
        ...
        if actual_reward is None:
            raise AssertionError(
                "Captured reward span does not contain a reward value."
            )
...
def _run_otel_reward_test(mode: str) -> dict[str, Any]:
    ...
    assert proc.exitcode == 0, (
        f"Subprocess for mode {mode!r} exited with code {proc.exitcode}"
    )
    return payload
