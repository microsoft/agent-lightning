...
def test_all_callbacks_log_without_raising():
    ...
    # Each callback should be callable with an event dict and not raise
    ...
    cb.on_state_saved({})
    cb.on_budget_updated({"metric_calls_remaining": 10})
    cb.on_error({"exception": "test error"})


def test_callbacks_accept_arbitrary_event_keys():
    cb = LightningGEPACallback()
    # All methods accept a dict, so any keys should be accepted
    cb.on_optimization_start({"extra": "value", "count": 42})
    cb.on_error({"exception": "message", "traceback": "stack", "severity": "high"})
