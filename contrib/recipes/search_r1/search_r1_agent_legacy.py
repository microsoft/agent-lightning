...
class Searchr1Agent(LitAgent[Any]):
    def __init__(self):
        super().__init__()
        self._is_main_process = os.environ.get("RANK", "0") == "0"
    ...
    async def training_rollout_async(
        ...
        while turn_id < 4 and not finished_flag:
            ...
            if len(turn_env_feedback) == 0:
                finished_flag = True
            self._log(
                f"===TURN ID=== {turn_id} \n ===RESP:=== {turn_response} \n ===ENV FEEDBACK=== {turn_env_feedback}"
            )
        ...
        if not finished_flag:
            ...
            self._log(f"======== LAST TURN GENERATE | RESP: {turn_response}")
...
if __name__ == "__main__":
    Trainer(n_workers=128).fit_v0(Searchr1Agent(), "http://localhost:9999/")
