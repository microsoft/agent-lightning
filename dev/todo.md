...
| 14 | **Configuration**: `deploy/.env` (secrets + bootstrap: `AGL_KEY`, `AGL_K8S_NAMESPACE`) and `deploy/config.yaml` (structured YAML: agl_lite_url, secret_name, controller settings). Deploy script creates ConfigMap with both the YAML file (`--from-file`) and extracted values as literal keys (`--from-literal`) for env var consumption. `AGL_K8S_NAMESPACE` lives only in `.env` — single source of truth, injected into ConfigMap by deploy script. |
...
| 18 | **Deploy scripts**: Bash for both deploy (`scripts/deploy.sh` — namespace, secrets, configmap, manifests, wait, health check) and image builds (`scripts/build_images.sh`). PoC orchestration in `examples/math-poc/run.py` (Python — needs port-forward lifecycle, AglLiteClient). |
