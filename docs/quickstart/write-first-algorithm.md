...
async def find_best_prompt(store, prompts_to_test, task_input):
    ...
    for prompt in prompts_to_test:
        ...
        reward_span = next((s for s in spans if s.name == "reward"), None)
        final_reward = float(reward_span.attributes.get("value", 0.0))
