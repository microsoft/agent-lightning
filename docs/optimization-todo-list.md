...
| 2 | 统一状态枚举和文档中的命名（`queuing`、`requeuing`、attempt/rollout 状态） | P0 | 状态模型 | 待开始 | 同步新增：状态枚举、查询参数与文档命名测试 | 先修正 `queueing`/`queuing` 拼写与语义混乱问题 |
| 3 | 修复 Attempt 终态模型（支持 `cancelled`，移除 `queuing`/`requeuing`） | P0 | 运行状态 | 待开始 | 同步新增：attempt/state contract tests | `timeout`、`unresponsive` 只属于 attempt；`requeuing` 只属于 rollout |
| 4 | 修复 `LitAgentRunner._step_impl()` 资源获取失败分支写入 | P0 | Runner | 待开始 | 同步新增：资源失败分支状态写入集成测试 | 资源获取失败必须走状态机并给出明确 attempt/rollout 终态 |
| 5 | 修复 `CancelledError` 处理，避免误判为 `succeeded` | P0 | Runner | 待开始 | 同步新增：runner cancel/shutdown 测试 | 单独捕获取消并映射到 `cancelled` 或失败终态 |
| 6 | 统一批量 span 写入返回值为 `SpanWriteResult(inserted/duplicates/failed)` | P0 | Store | 待开始 | 同步新增：collection/client/server 写入结果测试 | 替换当前不可区分的 fallback 写入结果；不再承诺跨后端 all-or-nothing |
...
| 8 | 删除 `ReadableSpan` 直接写入路径，避免重复 span | P0 | Tracer/Runner | 待开始 | 同步新增：span 重放、重复写入与污染测试 | OTEL span 只由 Tracer 侧写入 |
| 9 | 修复 `Runner.run_context(worker_id=...)` 初始化/teardown worker_id 不一致 | P0 | Runner | 待开始 | 同步新增：非 0 worker 上下文测试 | 初始化和清理使用同一 resolved worker id |
| 10 | 抽取并共享 `SpanWriter`，统一 `LightningSpanProcessor` 与 `LLMProxy` 导出行为 | P0 | Tracer/LLMProxy | 待开始 | 同步新增：writer timeout、失败日志、shutdown 测试 | 写入确认、超时和关闭语义只能有一处实现 |
| 11 | 清理 `agentlightning.__init__` 的 deprecated 导出 | P1 | API | 待开始 | 同步更新：公共 API 回归测试 | 移除 `AgentLightningClient`、`AgentLightningServer`、`configure_logger` 等旧符号 |
...
| 22 | 清理 legacy 类型传播（`RolloutLegacy`、`Task`、`TaskIfAny`、`RolloutRawResultLegacy`、`ReadableSpan` result path） | P1 | Types | 待开始 | 同步新增：类型导入清理扫描 | 运行时仅保留新 rollout/attempt/span/resource 模型 |
