...
| 3 | 修复 Attempt 终态模型（支持 `cancelled`，移除 `queuing`/`requeuing`） | P0 | 运行状态 | 进行中 | 同步新增：attempt/state contract tests | `timeout`、`unresponsive` 只属于 attempt；`requeuing` 只属于 rollout |
