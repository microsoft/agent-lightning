...
#### Step 2: Install vLLM and FlashAttention

Agent Lightning uses `verl` as its training backend. The compatible versions of `verl`, `vllm`, and `torch` are tightly coupled, and installing `flash-attn` can also be error-prone. We recommend using `scripts/setup_verl.sh` to install the tested, pinned GPU stack.

Pass the verl version and CUDA wheel variant explicitly. The script supports verl `0.7.1` or `0.8.0`, and CUDA wheel variant `cu129` or `cu130`. We recommend CUDA `13.0` with verl `0.8.0`:
