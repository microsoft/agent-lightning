...
def _normalize_model_endpoint(address: str) -> str:
    return address if address.startswith("http") else f"http://{address}/v1"


async def _replace_model_endpoints(
    client: AglLiteClient,
    model_name: str,
    server_addresses: list[str],
) -> None:
    """Replace the gateway's model pool with the current vLLM endpoints."""
    endpoints = [_normalize_model_endpoint(addr) for addr in server_addresses]
    try:
        await client.delete_model(model_name)
    except AglLiteError as exc:
        if exc.status_code != 404:
            raise
    if not endpoints:
        return
    await client.register_models([RegisterModelRequest(model=model_name, endpoint=endpoint) for endpoint in endpoints])
...
class AglLiteRolloutBridge:
    ...
    async def _async_run_agl_lite_rollout(
        ...
        async with AglLiteClient(base_url=self._agl_base_url, agl_key=self._agl_key) as client:
            await self._async_register_and_enqueue(client, data, server_addresses, is_train, rollout_metadata)
    ...
    async def _async_register_and_enqueue(
        ...
        await _replace_model_endpoints(client, model_name, server_addresses)
    ...
    def _build_enqueue_requests(
        ...
        *,
        ...
        base_metadata = dict(_to_native(rollout_metadata or {}))
        keys = list(data.keys())
        if not keys:
            return []
    ...
    def set_up_data_and_server(
        ...
        future = asyncio.run_coroutine_threadsafe(
            self._async_run_agl_lite_rollout(data, server_addresses, is_train, rollout_metadata), self._loop
        )
        future.result()
    ...
    def get_test_metrics(self):
        ...
        def append_sample_stat(stat: dict[str, Any], data_source: str | None) -> None:
            sample_stat_list.append(stat)
            if data_source is not None:
                sample_stat_list_by_source[data_source].append(stat)

        for rollout_id in self._enqueue_order:
            original_sample = self._task_id_to_original_sample.get(rollout_id, {})
            data_source = (
                _safe_metric_name(original_sample["data_source"]) if "data_source" in original_sample else None
            )
            ...
            metric_dict.update(
                {
                    ...
                    if stats_w_trace_by_source[data_source]
                    else 0.0,
    ...
    async def _async_register_and_enqueue_diff(
        ...
        """
        # Always replace the current vLLM endpoints so weight-swap between
        # steps lands on the correct addresses.
        model_name = self.train_information.get("model", "default-model")
        await _replace_model_endpoints(client, model_name, server_addresses)
        ...
        async def _go() -> int:
            async with AglLiteClient(
                ...
                return await self._async_register_and_enqueue_diff(
                    client, data, server_addresses, async_train_batch_size, rollout_metadata
                )
