...
class APO(BaseAlgorithm, Generic[T_task]):
    ...
    def _resolve_prefix(
        ...
        *,
        ...
        if prefix:
            return prefix
        return self._format_log_prefix(round_num=round_num, beam_idx=beam_idx, prompt_version=prompt_version)
    ...
    async def compute_textual_gradient(
        ...
        *,
        ...
        effective_prefix = self._resolve_prefix(
            prefix,
            round_num=round_num,
            beam_idx=beam_idx,
    ...
    async def _generate_candidate_prompts(
        ...
        self._log(
            logging.INFO,
            f"Applying {self.branch_factor} edits to each of the {len(parent_prompts)} parents on training dataset",
            prefix=round_prefix,
        )

        candidates: List[VersionedPromptTemplate] = []
        for beam_idx, prompt in parent_prompts:
    ...
    async def _evaluate_and_select_beam(
        ...
        self._log(
            logging.INFO,
            f"Top {len(sorted_prompts)} candidates on validation dataset: {selected_versions}",
            prefix=round_prefix,
        )
    ...
    async def _update_best_prompt(
        ...
        display_round = round_num + 1
        best_prompt = beam[0]
        prefix = self._format_log_prefix(round_num=display_round, beam_idx=1, prompt_version=best_prompt.version)
